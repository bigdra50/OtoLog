...
開始・停止・中断・失敗の経緯は `~/.local/state/otolog/recording.log`（`$XDG_STATE_HOME` を設定していればその下）にも残るため、アプリを再起動した後でも原因を確かめられる。
...
- meta.json の `endReason` は止めた記録で `stopped`、失敗で閉じた記録で `failed` になる。失敗のときは `endMessage` にポップオーバーと同じ理由が入る
