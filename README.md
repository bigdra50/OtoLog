...
| 保存先 / claude パス / 無音で自動停止 / 停止時の自動処理 / ログイン時起動 | ポップオーバー内の「設定」（記録中は保存先を変更できない） |
...
- Mac がスリープしている間も無音として数える。設定の時間より長くスリープしていた記録は、復帰した直後に止まる
- 無音かどうかは15秒ごとに確かめるため、止まるのは設定の時間より最大15秒遅れる

止め方は「停止」を押したときと同じで、記録した分を保存してセッションを閉じ、「停止時の自動処理」も走る。
発話（上の基準で、文字か数字を含む文字起こし）が1件も無いまま止まったときは、自動処理を走らせない（タイトル生成が必ず失敗するため）。
...
- タイトルは停止後に claude で自動生成できる（設定「停止時の自動処理」）。手動は「生成」内の「タイトル生成」から。**ディレクトリ名がタイトルにリネーム**され、meta.json・md 見出しもまとめて更新される
  - 発話（文字か数字を含む文字起こし）が1件も無いセッションには、停止時の自動処理を走らせない。停止ボタンで止めたときも同じ
